...
def test_parse_filename():
    ...
    sequences = FileSequence.from_components_in_filename_list(Components(), files)
    ...
    sequences = FileSequence.from_components_in_filename_list(Components(prefix="frame"), files)
    assert len(sequences) == 3  # frame.*.png, frame.*.jpg, frame_*.jpg

    sequences = FileSequence.from_components_in_filename_list(Components(extension="exr"), files)
    # image.*.exr, render.*.beauty.exr, render.*.spec.exr
    assert len(sequences) == 3

    # File extension tests
    sequences = FileSequence.from_components_in_filename_list(Components(extension="dpx"), files)
    assert len(sequences) == 2  # comp_v001.*.dpx, comp_v002.*.dpx

    sequences = FileSequence.from_components_in_filename_list(Components(extension="tif"), files)
    assert len(sequences) == 2  # shot_010_*.tif, shot_020_*.tif

    sequences = FileSequence.from_components_in_filename_list(Components(extension="tga"), files)
    assert len(sequences) == 1  # BG.*.tga

    # Prefix matching tests
    sequences = FileSequence.from_components_in_filename_list(Components(prefix="render"), files)
    assert len(sequences) == 3 

    sequences = FileSequence.from_components_in_filename_list(Components(prefix="BG"), files)
    assert len(sequences) == 2  # BG.*.tga, BG.*.jpg

    # Delimiter tests
    sequences = FileSequence.from_components_in_filename_list(Components(delimiter="."), files)
    assert len(sequences) == 10  # All sequences with dot delimiter

    sequences = FileSequence.from_components_in_filename_list(Components(delimiter="_"), files)
    assert len(sequences) == 3  # All sequences with underscore delimiter

    # Combined criteria tests
    sequences = FileSequence.from_components_in_filename_list(
    ...
    # Suffix tests
    sequences = FileSequence.from_components_in_filename_list(Components(suffix=".beauty"), files)
    assert len(sequences) == 1  # render.*.beauty.exr

    sequences = FileSequence.from_components_in_filename_list(Components(suffix=".spec"), files)
    assert len(sequences) == 1  # render.*.spec.exr

    # Multiple criteria tests
    sequences = FileSequence.from_components_in_filename_list(Components(
    ...
    # No match tests
    sequences = FileSequence.from_components_in_filename_list(Components(prefix="nonexistent"), files)
    assert len(sequences) == 0

    sequences = FileSequence.from_components_in_filename_list(Components(extension="missing"), files)
    assert len(sequences) == 0

    sequences = FileSequence.from_components_in_filename_list(Components(suffix="none"), files)
    assert len(sequences) == 0
